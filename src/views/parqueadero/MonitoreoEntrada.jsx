 ...
 * MonitoreoEntrada
 *
 * Vista de "Monitoreo de entrada": captura una foto (cámara o archivo),
 * la envía al endpoint OCR configurado en VITE_OCR_ENDPOINT y muestra el
 * resultado del reconocimiento de placa en tiempo real.
 *
 * Estados del flujo de la columna izquierda:
 *   vacio → camara → lista → procesando → resultado
...
const MonitoreoEntrada = () => {
  ...
  const detectarPlaca = async () => {
    ...
    try {
      ...
      // El endpoint del ingeniero ya ejecuta el OCR y consulta su propia BD.
      // La respuesta se pasa directamente a la UI, sin consultar Supabase local.
      setResultado(data)
