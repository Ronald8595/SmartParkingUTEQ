...
import { CBadge, CModal, CModalBody, CModalHeader, CModalTitle, CSpinner } from '@coreui/react'
 ...
 * endpoint del ingeniero; esta vista no consulta Supabase.
 *
 * @param {'vacio'|'camara'|'lista'|'procesando'|'resultado'|'error'} estadoFlujo
 * @param {object|null} resultado - Respuesta cruda del endpoint OCR.
 * @param {string} mensajeError - Mensaje ya traducido de un error HTTP/red.
 */
const ResultadoReconocimiento = ({ estadoFlujo, resultado, mensajeError }) => {
  ...
  if (clasificacion === 'encontrado') {
    const autorizado = /^(?!.*no\s).*autorizad/i.test(String(autorizacion ?? ''))
      ...
      <div className="d-flex flex-column gap-3">
        ...
        <div className="border rounded overflow-hidden">
          ...
          <div className="d-flex justify-content-between align-items-center px-3 py-2">
            ...
            {autorizacion ? (
              <CBadge color={autorizado ? 'success' : 'danger'}>{autorizacion}</CBadge>
            ) : (
              <span className="fw-bold text-body-tertiary">—</span>
            )}
          </div>
